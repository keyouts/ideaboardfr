...
function updateTagFilterDropdown() {
  ...
  allTags.forEach(tag => {
    ...
    option.value = tag;
    option.textContent = tag === 'all' ? 'Afficher tout' : tag;
...
function getTagOptionsHTML(selectedTag = '') {
  let options = `<option value="">Aucun tag</option>`;
  customTags.forEach(tag => {
...
function createNote(id, content = '', x = 100, y = 100, image = '', tag = '') {
  ...
  note.innerHTML = `
    <textarea placeholder="Cliquez pour éditer">${content}</textarea>
    ${image ? `<img src="${image}" />` : ''}
    <div class="controls">
      <button class="addImage">Ajouter une image</button>
      <button class="selectNote">Sélectionner</button>
